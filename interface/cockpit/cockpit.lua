...
function fuelCost(travel)
  self.systemA = celestial.currentSystem()
  self.systemB = {location = travel or self.travel.system, planet = 0, satellite = 0, system = self.travel.target}

  local distance = math.sqrt((self.systemA.location[1] - self.systemB.location[1])^2 + (self.systemA.location[2] - self.systemB.location[2])^2)

  local cost = math.min(config.getParameter("jumpFuelCost") * 2^math.floor(distance/50), 5000) --every 50 units of distance, multiplies the fuel cost by 2 exponentially. fuel cost caps at 5000
