...
function init()
  ...
  self.projectileInterval = config.getParameter("projectileInterval")
  self.projectileCount = config.getParameter("projectileCount", 3)
  self.projectileInaccuracy = config.getParameter("projectileInaccuracy")
...
function fireBurst(dt)
  local shots = self.projectileCount
  while shots > 0 do 
      shots = shots - 1
      fireProjectile()
  end
...
function fireProjectile(projectileType, projectileParams, inaccuracy, firePosition, projectileCount)
  ...
  projectileId = world.spawnProjectile(
      ...
      {sb.nrand(self.projectileInaccuracy), 1},
      false,
      params
    )
...
function fireExplosion(projectileType, projectileParams, inaccuracy, firePosition, projectileCount)
  local params = sb.jsonMerge(self.projectileParameters, projectileParams or {})
  params.power = 0
  params.powerMultiplier = activeItem.ownerPowerMultiplier()
  params.speed = util.randomInRange(params.speed)
  ...
  if not projectileType then
    projectileType = "poisonexplosionknockback"
  end
  if type(projectileType) == "table" then
    projectileType = projectileType[math.random(#projectileType)]
  end
